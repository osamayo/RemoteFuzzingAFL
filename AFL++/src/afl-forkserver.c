...
void afl_fsrv_start(afl_forkserver_t *fsrv, char **argv,
  ...
  if (fsrv->remote_fuzzing)
  {
    ...
    puts("success!");
    // FeedbackCondVar ret = init_cond_var();
    // slock = ret.mutex;
    // scond = ret.cond;
    // srun_result = ret.run_result;
    // // malloc testcases buffer
    // fsrv->testcases = (u8**) malloc(sizeof(u8*) * fsrv->bufferLen);
    // fsrv->testcasesBuffer = (u8**) malloc(sizeof(u8*) * fsrv->bufferLen);
    // fsrv->feedbacksBuffer = (u8**) malloc(sizeof(u8*) * fsrv->bufferLen);

    // if (fsrv->testcasesBuffer == NULL || fsrv->testcases == NULL || fsrv->feedbacksBuffer == NULL)
    // {
    //     perror("Error while malloc()");
    //     exit(EXIT_FAILURE);
    // }
    // for (int i=0; i<fsrv->bufferLen; i++)
    // {
    //     *(fsrv->testcases+i) = (u8*) malloc(MAX_FILE);
    //     *(fsrv->testcasesBuffer+i) = (u8*) malloc(MAX_FILE);
    //     *(fsrv->feedbacksBuffer+i) = (u8*) malloc(MAX_FILE);

    //     if (*(fsrv->testcases+i) == NULL || *(fsrv->testcasesBuffer+i) == NULL || *(fsrv->feedbacksBuffer +i) == NULL)
    //     {
    //         perror("Error while malloc()");
    //         exit(EXIT_FAILURE);
    //     }
    ...
    // fsrv->testcasesLengthArr = (u32*) malloc (sizeof(u32) * fsrv->bufferLen);
    // fsrv->testcasesBufferLengthArr = (u32*) malloc (sizeof(u32) * fsrv->bufferLen);
    // fsrv->feedbacksLengthArr = (u32*) malloc (sizeof(u32) * fsrv->bufferLen);
    // fsrv->feedbacksFaultCodes = (u8*) malloc(sizeof(u8) * fsrv->bufferLen);

    // if (fsrv->testcasesLengthArr == NULL || fsrv->testcasesBufferLengthArr == NULL || fsrv->feedbacksLengthArr == NULL || fsrv->feedbacksFaultCodes == NULL)
    // {
    //     perror("Error while malloc()");
    //     exit(EXIT_FAILURE);
    // }
    ...
    // for (int i=0; i<fsrv->bufferLen; i++)
    // {
    //   *(fsrv->testcasesLengthArr + i) = 0;
    //   *(fsrv->testcasesBufferLengthArr + i) = 0;
    //   *(fsrv->feedbacksLengthArr + i) = 0;
    // }
    
    
    // fsrv->ready = true;
    // fsrv->cond = (pthread_cond_t)PTHREAD_COND_INITIALIZER;
    // int rc = pthread_mutex_init(&fsrv->mutex, NULL);
    // assert(rc==0);
...
u32 afl_fsrv_get_mapsize(afl_forkserver_t *fsrv, char **argv,
  ...
  printf("map_size: %d | real_map_size: %d\n", fsrv->map_size, fsrv->real_map_size);
...
afl_fsrv_write_to_testcase(afl_forkserver_t *fsrv, u8 *buf, size_t len) {
  ...
  // fsrv->testcasesHash +=  hash64(buf, len, HASH_CONST);
  // printf("testcases: %d\n", fsrv->testcasesHash);
  if (fsrv->remote_fuzzing)
  {
    ...
    fsrv->testcase = buf;
    // FILE* log= fopen("/home/kali/Desktop/fuzz.txt", "a");
    // fprintf(log, "testcase %d: %d\n", fsrv->total_execs+1, hash64(buf, len, HASH_CONST));
    // fflush(log);
    // fclose(log);
    return;
  }
