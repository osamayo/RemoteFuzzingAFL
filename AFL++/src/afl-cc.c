...
void add_defs_persistent_mode(aflcc_state_t *aflcc) {
  ...
  bool IsRemoteFuzzing = false;
  if (getenv("AFL_REMOTE_FUZZ"))
  {
    printf("Remote in-process fuzzing!\n");
    IsRemoteFuzzing = true;
    // insert_object(aflcc, "afl-performance.o", 0, 0);
  ...
  if (IsRemoteFuzzing)
  {
    ...
    insert_param(aflcc,
               "-D__AFL_FUZZ_TESTCASE_BUF=(__afl_fuzz_ptr)");

    insert_param(
        aflcc,
        "-D__AFL_FUZZ_TESTCASE_LEN=(*__afl_fuzz_len)");
    ...
    switch (aflcc->bit_mode) {
      ...
      case 0:
        if (!aflcc->shared_linking && !aflcc->partial_linking)
        {
          if (getenv("AFL_REMOTE_FUZZ"))
          {
            insert_object(aflcc, "afl-compiler-remotefuzz-rt.o", 0, 0);
            insert_object(aflcc, "afl-remotefuzz-communication.o", 0, 0);
          }
          else
      ...
      case 32:
        if (!aflcc->shared_linking && !aflcc->partial_linking)
        {
          if (getenv("AFL_REMOTE_FUZZ"))
          {
            insert_object(aflcc, "afl-compiler-remotefuzz-rt-32.o", 0, 0);
            insert_object(aflcc, "afl-remotefuzz-communication-32.o", 0, 0);
          }else
