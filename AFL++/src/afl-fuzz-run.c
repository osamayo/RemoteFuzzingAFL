...
#endif
  // printf("fuzz_run_target return: %d\n", res);
  return res;
...
static void write_with_gap(afl_state_t *afl, u8 *mem, u32 len, u32 skip_at,
  ...
  */
  ...
  } else if (likely(afl->fsrv.use_shmem_fuzz)) {
...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  // }
    ...
    if (q->exec_cksum != cksum) {
      // printf("cksum %llu | %llu\n", q->exec_cksum, cksum);
      // if (q->exec_cksum !=0 ) FATAL("stop!");
      hnb = has_new_bits(afl, afl->virgin_bits);
      if (hnb > new_bits) { new_bits = hnb; }
      ...
      if (q->exec_cksum) {
        ...
        for (i = 0; i < afl->fsrv.map_size; ++i) { // TODO: remotely
        ...
        q->exec_cksum = cksum;
        memcpy(afl->first_trace, afl->fsrv.trace_bits, afl->fsrv.map_size); // request the server to update first_trace
